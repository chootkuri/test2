...
for index in range(10):
    #-- read a label
    label = mnist_label.readline().strip()  # 라벨에서 공백 제거
    print(label)
    ...
    y_pred_prob = model.predict(im2arr)  # 클래스별 확률을 예측
    y_pred_class = np.argmax(y_pred_prob)  # 가장 높은 확률을 갖는 클래스 선택
    print(y_pred_class)
    
    #-- compute the accuracy of the prediction
    if int(label) == y_pred_class:
        cnt_correct += 1
    num_samples += 1
...
#-- Final accuracy
Final_acc = cnt_correct / num_samples
print()
